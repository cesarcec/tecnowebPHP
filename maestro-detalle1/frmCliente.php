...
<body>
    ...
    <td>
        ...
        if (isset($_GET['cod'])) {
            ...
            $cod = ''; // Valor por defecto si 'cod' no está definido
        }
        ?>
        <input name="txtIdCliente" type="hidden" value="<?php echo $cod; ?>" id="txtIdCliente" />
					...
					<tr>
						...
						<td width="227"><label>
								...
								if (isset($_GET['nom']))
									$nom = $_GET['nom'];
								?>
								<input name="txtNombre" type="text" value="<?php if (isset($_GET['nom'])) {
																				echo $nom;
																			} ?>" id="txtNombre" />
							</label></td>
					</tr>
					<tr>
						...
						<td><label>
								...
								if (isset($_GET['ape']))
									$ape = $_GET['ape'];
								?>
								<input name="txtApellidos" type="text" value="<?php if (isset($_GET['ape'])) {
																					echo $ape;
																				} ?>" id="txtApellidos" />
							</label></td>
					</tr>
					<tr>
						...
						<td><label>
								...
								if (isset($_GET['emp']))
									$emp = $_GET['emp'];
								?>
								<input name="txtEmpresa" type="text" value="<?php if (isset($_GET['emp'])) {
																				echo $emp;
																			} ?>" id="txtEmpresa" />
							</label></td>
					</tr>
					<tr>
						...
						<td><label>
								...
								if (isset($_GET['tel']))
									$tel = $_GET['tel'];
								?>
								<input name="txtTelefono" type="text" value="<?php if (isset($_GET['tel'])) {
																					echo $tel;
																				} ?>" id="txtTelefono" />
							</label></td>
					</tr>
					<tr>
						...
						<td><label>
								...
								if (isset($_GET['dir']))
									$dir = $_GET['dir'];
								?>
								<input name="txtDireccion" type="text" value="<?php if (isset($_GET['dir'])) {
																					echo $dir;
																				} ?>" id="txtDireccion" />
							</label></td>
					</tr>
					...
					<tr>
						<td colspan="2">&nbsp;<br>
							...
							?>
								<label>
									<input name="grupo" type="radio" value="1" checked="checked" <?php if (isset($_GET['grupo']) && $_POST['grupo'] == "1") echo "checked"; ?> />
									Apellidos</label>
								<label>
									<input type="radio" name="grupo" <?php if (isset($_GET['grupo']) && $_POST['grupo'] == "2") echo "checked"; ?> value="2" />
									Empresa</label>
								<label>
									<input name="txtBuscar" type="text" id="txtBuscar" value="<?php if (isset($_GET['txtBuscar'])) {
																									echo $valor;
																								} ?>" size="33" />
								</label>
								<label></label>
	...
	function eliminar() {
		if (isset($_POST['txtIdCliente']) && !empty($_POST['txtIdCliente'])) {
			...
			echo "Para eliminar el cliente, debe introducir el ID del cliente";
		}
	}
